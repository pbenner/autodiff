...
func (r *SparseFloat32Matrix) MaddS(a ConstMatrix, b ConstScalar) Matrix {
  n, m := r.Dims()
  n1, m1 := a.Dims()
  if n1 != n || m1 != m {
    panic("matrix dimensions do not match!")
...
func (r *SparseFloat32Matrix) MsubS(a ConstMatrix, b ConstScalar) Matrix {
  n, m := r.Dims()
  n1, m1 := a.Dims()
  if n1 != n || m1 != m {
    panic("matrix dimensions do not match!")
...
func (r *SparseFloat32Matrix) MmulS(a ConstMatrix, b ConstScalar) Matrix {
  n, m := r.Dims()
  n1, m1 := a.Dims()
  if n1 != n || m1 != m {
    panic("matrix dimensions do not match!")
...
func (r *SparseFloat32Matrix) MdivM(a, b ConstMatrix) Matrix {
  ...
  for i := 0; i < n; i++ {
    for j := 0; j < m; j++ {
      c1 := a.ConstAt(i, j)
      c2 := b.ConstAt(i, j)
      if c1.GetFloat32() != float32(0) || c2.GetFloat32() == float32(0) {
        r.At(i, j).Div(c1, c2)
      } else {
        if r.ConstAt(i, j).GetFloat32() != 0.0 {
          r.At(i, j).Reset()
        }
      }
    }
...
func (r *SparseFloat32Matrix) MdivS(a ConstMatrix, b ConstScalar) Matrix {
  n, m := r.Dims()
  n1, m1 := a.Dims()
  if n1 != n || m1 != m {
    panic("matrix dimensions do not match!")
  }
  if b.GetFloat32() == float32(0) {
    for i := 0; i < n; i++ {
      for j := 0; j < m; j++ {
...
func (r *SparseFloat32Matrix) Jacobian(f func(ConstVector) ConstVector, x_ MagicVector) Matrix {
  n, m := r.Dims()
...
func (r *SparseFloat32Matrix) Hessian(f func(ConstVector) ConstScalar, x_ MagicVector) Matrix {
  n, m := r.Dims()
  // reallocate matrix if dimensions do not match
