...
func (r *SparseInt8Matrix) MaddS(a ConstMatrix, b ConstScalar) Matrix {
  n, m := r.Dims()
  n1, m1 := a.Dims()
  if n1 != n || m1 != m {
    panic("matrix dimensions do not match!")
...
func (r *SparseInt8Matrix) MsubS(a ConstMatrix, b ConstScalar) Matrix {
  n, m := r.Dims()
  n1, m1 := a.Dims()
  if n1 != n || m1 != m {
    panic("matrix dimensions do not match!")
...
func (r *SparseInt8Matrix) MmulS(a ConstMatrix, b ConstScalar) Matrix {
  n, m := r.Dims()
  n1, m1 := a.Dims()
  if n1 != n || m1 != m {
    panic("matrix dimensions do not match!")
...
func (r *SparseInt8Matrix) MdivM(a, b ConstMatrix) Matrix {
  ...
  for i := 0; i < n; i++ {
    for j := 0; j < m; j++ {
      c1 := a.ConstAt(i, j)
      c2 := b.ConstAt(i, j)
      if c1.GetInt8() != int8(0) || c2.GetInt8() == int8(0) {
        r.At(i, j).Div(c1, c2)
      } else {
        if r.ConstAt(i, j).GetInt8() != 0.0 {
          r.At(i, j).Reset()
        }
      }
    }
...
func (r *SparseInt8Matrix) MdivS(a ConstMatrix, b ConstScalar) Matrix {
  n, m := r.Dims()
  n1, m1 := a.Dims()
  if n1 != n || m1 != m {
    panic("matrix dimensions do not match!")
