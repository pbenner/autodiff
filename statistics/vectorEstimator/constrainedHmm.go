...
func NewConstrainedHmmEstimator(pi Vector, tr Matrix, stateMap, startStates, finalStates []int, tree generic.HmmNode, estimators []ScalarEstimator, epsilon float64, maxSteps int, args... interface{}) (*HmmEstimator, error) {
  if hmm, err := vectorDistribution.NewConstrainedHmm(pi, tr, stateMap, nil, tree); err != nil {
    return nil, err
  } else {
