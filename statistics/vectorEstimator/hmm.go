...
func (obj *HmmEstimator) SetData(x []ConstVector, n int) error {
  ...
  if data, err := NewHmmStdDataSet(obj.ScalarType(), x, obj.hmm1.NEDists()); err != nil {
    ...
    for i, estimator := range obj.estimators {
      ...
      // initialize distribution
      obj.hmm1.Edist[i] = estimator.GetEstimate().CloneScalarPdf()
      obj.hmm2.Edist[i] = estimator.GetEstimate().CloneScalarPdf()
      obj.hmm3.Edist[i] = estimator.GetEstimate().CloneScalarPdf()
    }
    obj.data = data
  }
  return nil
...
func (obj *HmmEstimator) GetEstimate() VectorPdf {
  return obj.hmm1
}
