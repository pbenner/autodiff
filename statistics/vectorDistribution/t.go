...
func (dist *TDistribution) SetParameters(parameters Vector) error {
  n := dist.Dim()
  nu    := parameters.At   (0)  ; parameters = parameters.Slice(1, parameters.Dim())
  mu    := parameters.Slice(0,n); parameters = parameters.Slice(n, parameters.Dim())
...
func (obj *TDistribution) ImportConfig(config ConfigDistribution, t ScalarType) error {
  ...
  nu, ok := config.GetNamedParameterAsScalar("Nu", t); if !ok {
    return fmt.Errorf("invalid config file")
  }
