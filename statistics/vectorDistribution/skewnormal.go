...
func (obj *SkewNormalDistribution) ImportConfig(config ConfigDistribution, t ScalarType) error {
  ...
  xi, ok := config.GetNamedParametersAsVector("Xi", t); if !ok {
    return fmt.Errorf("invalid config file")
  }
  omega, ok := config.GetNamedParametersAsMatrix("Omega", t, n, n); if !ok {
    return fmt.Errorf("invalid config file")
  }
  alpha, ok := config.GetNamedParametersAsVector("Alpha", t); if !ok {
    return fmt.Errorf("invalid config file")
  }
  scale, ok := config.GetNamedParametersAsVector("Scale", t); if !ok {
    return fmt.Errorf("invalid config file")
  }
