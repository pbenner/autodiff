...
func (obj *MixtureEstimator) SetData(x []ConstMatrix, n int) error {
  if data, err := NewMixtureStdDataSet(obj.ScalarType(), x, obj.mixture1.NComponents()); err != nil {
    ...
    for i, estimator := range obj.estimators {
      ...
      // initialize distribution
      obj.mixture1.Edist[i] = estimator.GetEstimate().CloneMatrixPdf()
      obj.mixture2.Edist[i] = estimator.GetEstimate().CloneMatrixPdf()
      obj.mixture3.Edist[i] = estimator.GetEstimate().CloneMatrixPdf()
    }
    obj.data = data
  }
  return nil
...
func (obj *MixtureEstimator) GetEstimate() MatrixPdf {
  return obj.mixture1
}
