...
func NewHierarchicalHmmEstimator(pi Vector, tr Matrix, stateMap, startStates, finalStates []int, tree generic.HmmNode, estimators []VectorEstimator, epsilon float64, maxSteps int, args ...interface{}) (*HmmEstimator, error) {
	if hmm, err := matrixDistribution.NewHierarchicalHmm(pi, tr, stateMap, nil, tree); err != nil {
		return nil, err
	} else {
