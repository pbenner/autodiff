...
/* -------------------------------------------------------------------------- */
func cholesky_float32(A *DenseFloat32Matrix, L *DenseFloat32Matrix, s, t Float32) (*DenseFloat32Matrix, *DenseFloat32Matrix, error) {
  n, _ := A.Dims()
  for i := 0; i < n; i++ {
    for j := 0; j < (i+1); j++ {
      s.Reset()
...
func cholesky_ldl_float32(A *DenseFloat32Matrix, L, D *DenseFloat32Matrix, s, t Float32) (*DenseFloat32Matrix, *DenseFloat32Matrix, error) {
  n, _ := A.Dims()
  c := t
  for j := 0; j < n; j++ {
...
func cholesky_ldl_forcepd_float32(A *DenseFloat32Matrix, L, D *DenseFloat32Matrix, s, t Float32) (*DenseFloat32Matrix, *DenseFloat32Matrix, error) {
  n, _ := A.Dims()
  ...
  for i := 0; i < n; i++ {
    for j := 0; j < n; j++ {
      if i == j {
        if r := math.Abs(A.AT(i, i).GetFloat64()); r > gamma {
          gamma = r
        }
      } else {
        if r := math.Abs(A.AT(i, j).GetFloat64()); r > xi {
          xi = r
        }
      }
    }
