...
func (r *SparseInt8Vector) VADDV(a, b *SparseInt8Vector) *SparseInt8Vector {
  if n := r.Dim(); a.Dim() != n || b.Dim() != n {
    panic("vector dimensions do not match")
  }
...
func (r *SparseInt8Vector) VaddS(a ConstVector, b ConstScalar) Vector {
  n := r.Dim()
  if n != a.Dim() {
    panic("vector dimensions do not match")
  }
...
func (r *SparseInt8Vector) VADDS(a *SparseInt8Vector, b Int8) *SparseInt8Vector {
  r.VaddS(a, b)
  return r
}
/* -------------------------------------------------------------------------- */
...
func (r *SparseInt8Vector) VSUBV(a, b *SparseInt8Vector) *SparseInt8Vector {
  if n := r.Dim(); a.Dim() != n || b.Dim() != n {
    panic("vector dimensions do not match")
  }
  for it := r.JOINT3_ITERATOR_(a, b); it.Ok(); it.Next() {
...
func (r *SparseInt8Vector) VsubS(a ConstVector, b ConstScalar) Vector {
  n := r.Dim()
  if n != a.Dim() {
    panic("vector dimensions do not match")
  }
...
func (r *SparseInt8Vector) VSUBS(a *SparseInt8Vector, b Int8) *SparseInt8Vector {
  r.VsubS(a, b)
  return r
}
/* -------------------------------------------------------------------------- */
...
func (r *SparseInt8Vector) VMULV(a, b *SparseInt8Vector) *SparseInt8Vector {
  if n := r.Dim(); a.Dim() != n || b.Dim() != n {
    panic("vector dimensions do not match")
  }
  for it := r.JOINT3_ITERATOR_(a, b); it.Ok(); it.Next() {
...
func (r *SparseInt8Vector) VmulS(a ConstVector, b ConstScalar) Vector {
  if r.Dim() != a.Dim() {
    panic("vector dimensions do not match")
  }
...
func (r *SparseInt8Vector) VMULS(a *SparseInt8Vector, b Int8) *SparseInt8Vector {
  if r.Dim() != a.Dim() {
    panic("vector dimensions do not match")
  }
...
func (r *SparseInt8Vector) VDIVV(a, b *SparseInt8Vector) *SparseInt8Vector {
  r.VdivV(a, b)
  return r
}
/* -------------------------------------------------------------------------- */
// Element-wise division of a vector and a scalar. The result is stored in r.
func (r *SparseInt8Vector) VdivS(a ConstVector, b ConstScalar) Vector {
  n := r.Dim()
  if n != a.Dim() {
    panic("vector dimensions do not match")
  }
...
func (r *SparseInt8Vector) VDIVS(a *SparseInt8Vector, b Int8) *SparseInt8Vector {
  if r.Dim() != a.Dim() {
    panic("vector dimensions do not match")
  }
  for it := r.JOINT_ITERATOR_(a); it.Ok(); it.Next() {
...
func (r *SparseInt8Vector) MdotV(a ConstMatrix, b ConstVector) Vector {
  n, m := a.Dims()
  if r.Dim() != n || b.Dim() != m {
...
func (r *SparseInt8Vector) VdotM(a ConstVector, b ConstMatrix) Vector {
  n, m := b.Dims()
  if r.Dim() != m || a.Dim() != n {
    panic("matrix/vector dimensions do not match!")
  }
