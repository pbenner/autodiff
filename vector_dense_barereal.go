...
func (v DenseBareRealVector) Append(a ...Scalar) Vector {
  for _, s := range a {
    v = append(v, BareReal(s.GetValue()))
  }
  return v
}
...
func (v DenseBareRealVector) ConvertElementType(t ScalarType) {
}
...
func ImportDenseBareRealVector(t ScalarType, filename string) (DenseBareRealVector, error) {
  result := NewDenseBareRealVector([]float64{})
  ...
  for scanner.Scan() {
    ...
    for i := 0; i < len(fields); i++ {
      ...
      result = append(result, BareReal(value))
    }
  }
  return result, nil
}
