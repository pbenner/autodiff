...
func (r *SparseFloat64Matrix) MaddS(a ConstMatrix, b ConstScalar) Matrix {
  n, m := r.Dims()
  n1, m1 := a.Dims()
  if n1 != n || m1 != m {
    panic("matrix dimensions do not match!")
...
func (r *SparseFloat64Matrix) MsubS(a ConstMatrix, b ConstScalar) Matrix {
  n, m := r.Dims()
  n1, m1 := a.Dims()
  if n1 != n || m1 != m {
    panic("matrix dimensions do not match!")
...
func (r *SparseFloat64Matrix) MmulS(a ConstMatrix, b ConstScalar) Matrix {
  n, m := r.Dims()
  n1, m1 := a.Dims()
  if n1 != n || m1 != m {
    panic("matrix dimensions do not match!")
...
func (r *SparseFloat64Matrix) MdivM(a, b ConstMatrix) Matrix {
  ...
  for i := 0; i < n; i++ {
    for j := 0; j < m; j++ {
      c1 := a.ConstAt(i, j)
      c2 := b.ConstAt(i, j)
      if c1.GetFloat64() != float64(0) || c2.GetFloat64() == float64(0) {
        r.At(i, j).Div(c1, c2)
      } else {
        if r.ConstAt(i, j).GetFloat64() != 0.0 {
          r.At(i, j).Reset()
        }
      }
    }
...
func (r *SparseFloat64Matrix) MdivS(a ConstMatrix, b ConstScalar) Matrix {
  n, m := r.Dims()
  n1, m1 := a.Dims()
  if n1 != n || m1 != m {
    panic("matrix dimensions do not match!")
  }
  if b.GetFloat64() == float64(0) {
    for i := 0; i < n; i++ {
      for j := 0; j < m; j++ {
...
func (r *SparseFloat64Matrix) Jacobian(f func(ConstVector) ConstVector, x_ MagicVector) Matrix {
  n, m := r.Dims()
...
func (r *SparseFloat64Matrix) Hessian(f func(ConstVector) ConstScalar, x_ MagicVector) Matrix {
  n, m := r.Dims()
  // reallocate matrix if dimensions do not match
