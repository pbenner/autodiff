...
func (c *Real64) Sigmoid(a ConstScalar, t Scalar) Scalar {
  if a.GetFloat64() >= 0 {
    c.Neg(a)
    c.Exp(c)
    c.Add(c, ConstFloat64(1.0))
    c.Div(ConstFloat64(1.0), c)
  } else {
    t.Exp(a)
    c.Set(t)
...
func (r *Real64) Vmean(a ConstVector) Scalar {
  ...
  return r.Div(r, ConstFloat64(float64(a.Dim())))
}
...
func (r *Real64) Vnorm(a ConstVector) Scalar {
  ...
  for it := a.ConstIterator(); it.Ok(); it.Next() {
    t.Pow(it.GetConst(), ConstFloat64(2.0))
    r.Add(r, t)
  }
  r.Sqrt(r)
  return r
...
func (r *Real64) Mnorm(a ConstMatrix) Scalar {
  ...
  v := a.AsConstVector()
  r.Pow(v.ConstAt(0), ConstFloat64(2.0))
  for i := 1; i < v.Dim(); i++ {
    t.Pow(v.ConstAt(i), ConstFloat64(2.0))
    r.Add(r, t)
  }
  return r
}
